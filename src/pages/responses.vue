  ...
  <div>
                    ...
                    <div class="card-body">
                      <b-row>
                        ...
                        <b-col xl="2">
                          ...
                          <i data-feather="plus-square" > </i>Add response</b-button>  
                        </b-col>
                      </b-row>
                ...
                <px-card title="" :actions="false">
                  <div slot="with-padding">
                    <div class="row">
                      ...
                      <div class="col-xl-4 col-md-6 box-col-6" v-for="(response,index) in responses" :key="index">
                          ...
                          <div class="media">
                            <div class="media-body ml-3">
                              ...
                              <div class="price d-flex"> 
                                <div class="text-muted mr-2">{{ response.attributes.description }}</div>
                              </div>
                                <br><br>                             
                              <div class="avaiabilty">
                                
                              <a v-if="role != 'client'" class="btn btn-warning btn-xs" href="#">Update</a><feather v-if="role != 'client'" class="close" @click="deleteee(response.id)"  type="trash-2"></feather>
      ...
      <b-modal okTitle= '' cancelTitle= '' headerClass= 'p-2 border-bottom-0' footerClass = 'p-2 border-top-0' okVariant= 'seacndary' cancelVariant= 'seacndary' id="modal-lg" size="lg" title="" :ok-disabled="true" :cancel-disabled="true">
                        ...
                        <b-form-group id="example-input-group-5" label="image" label-for="example-input-5">
                            ...
                            <b-form-group id="example-input-group-2" label="Ticket" label-for="example-input-2">
                                  <multiselect  v-model="form.ticket_id" :options="tickets" label="title" :searchable="false" :close-on-select="true" :show-labels="true" placeholder="Pick a status"></multiselect>
...
<script>
  ...
  export default {
    ...
    created(){
      if (localStorage.getItem("role")) {
       axios.get('reponses').then((response)=>{
         console.log(response.data.reponses)
        this.responses=response.data.reponses
      })
      ...
      axios.get('showtickets').then((response)=>{
        ...
        this.role = "client"
        axios.get('getResponses').then((response)=>{
         console.log(response.data.reponses)
        this.responses=response.data.reponses
      })
      }
    ...
    methods:{
      ...
      add() {
        ...
        this.form.personnel_id = user.id
        this.form.ticket_id = this.form.ticket_id.id
        console.log(this.form);
         this.$validator.validateAll().then(result => {
          ...
          axios.post('reponses',this.form).then(res =>{
            if (res.data.status == 200){
              this.$toastr.s('Reponse added ');
              setTimeout(() => {
                location.reload();
              }, '500');
            } else{
 ...
 validateState(ref) {
      ...
      deleteee(id) {
      axios.delete('reponses/'+id).then(res =>{
              this.$toastr.s('Reponse deleted ! ');
              setTimeout(() => {
                location.reload();
              }, '500');
